...
public class MyLogAspect {
    ...
    private SysLogEntity sysLogEntitd;
    //切点
    ...
    public void doBeforeLog(JoinPoint joinPoint){
        log.info ("@Before:======方法进入了=====");
        //获得签名
        Signature signature = joinPoint.getSignature();
        //获取即将执行的方法名
        String method = signature.getName();
        log.info ("{},方法{},开始执行","@Before",method);
    }

    //前置执行
    @After ("pointoutLog()")
    public void doAfterLog(JoinPoint joinPoint){
        log.info ("@After:====方法进入了=======");
        Signature signature = joinPoint.getSignature();
        String method = signature.getName();
        sysLogEntitd=null;
        log.info ("{},方法{},结束执行","@After",method);

    }

    @Around("pointoutLog()")
    public Object doAround(ProceedingJoinPoint joinPoint) throws Throwable {
        log.info ("@Around:===================开始环绕增强处理===================");

        long beginTime = System.currentTimeMillis();
        long time = System.currentTimeMillis() - beginTime;
        Object proceed = joinPoint.proceed ();
        log.info ("@Around:===================结束环绕增强处理===================");
        /*try {
            SysLogEntity sysLogEntityd = sysLogEntitd;
            //实现保存日志逻辑
